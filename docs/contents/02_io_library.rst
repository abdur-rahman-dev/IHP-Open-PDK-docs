IO and Periphery Library (sg13g2_io)
=====================================
...
The ``sg13g2_io`` library provides IO pad cells for interfacing the 1.2V core logic domain
with the 3.3V IO domain.

The library files are located in ``$PDK_ROOT/ihp-sg13g2/libs.ref/sg13g2_io/``.

.. tip::
    The source code for the IO library generation can be found in the
    `c4m-pdk-ihpsg13g2 <https://gitlab.com/Chips4Makers/c4m-pdk-ihpsg13g2>`_ repository.
...
.. csv-table:: Signal Pads
    :header: "Cell", "Type", "Drive Strength", "Description"
    :stub-columns: 0
    :file: tables/io_signal_pads.csv
...
.. csv-table:: Power and Ground Pads
    :header: "Cell", "Voltage", "Description"
    :stub-columns: 0
    :file: tables/io_power_pads.csv
    :widths: 200, 100, 300
...
.. csv-table:: Fillers and Corners
    :header: "Cell", "Width", "Description"
    :stub-columns: 0
    :file: tables/io_fillers.csv
    :widths: 200, 100, 300
...
and the pads can physically drive loads up to these values. The ``max_capacitance`` constraint
appears to be conservatively set by the library generator
(`c4m-pdk-ihpsg13g2 <https://gitlab.com/Chips4Makers/c4m-pdk-ihpsg13g2>`_).
...
This includes the bond wire, package pin, PCB trace, connector, and input capacitance of
the receiving device.

Typical load capacitances:

- **Chip-to-chip on PCB**: 5-15 pF (trace + via + connector + receiver input)
- **Short wire bond to adjacent die**: < 2 pF
- **Test probe**: 1-3 pF

.. warning::
    The 4 mA variant has a ``max_capacitance`` of only 1.11 pF. A typical PCB trace
    presents 5-15 pF of capacitive load, which is well beyond this limit. In the slow
    process corner, drive capability drops to 2.3 mA, resulting in degraded logic levels.

**4 mA**
    Only suitable for very light loads (< 1 pF). Useful for chip-to-chip
    connections with minimal trace length, or for internal test points.

**16 mA**
    Suitable for moderate loads up to ~4 pF. Adequate for short PCB traces
    with well-controlled impedance.

**30 mA**
    Recommended for general-purpose PCB I/O. Provides the best noise margin
    across all process corners. Use this variant when the chip will be packaged
...
+------------+-----------+---------------------------------------------------+
| ``pad``    | output    | Pad connection to bond wire (3.3V domain)         |
+------------+-----------+---------------------------------------------------+
...
| Pin        | Direction | Description                                       |
+============+===========+===================================================+
| ``pad``    | input     | Pad connection from bond wire (3.3V domain)       |
+------------+-----------+---------------------------------------------------+
...
| ``pad``    | inout     | Pad connection to bond wire (3.3V domain)         |
+------------+-----------+---------------------------------------------------+
| ``padres`` | inout     | Resistive pad connection (direct pass-through)    |
+------------+-----------+---------------------------------------------------+
...
All data in this page is extracted from the following files in
``$PDK_ROOT/ihp-sg13g2/libs.ref/sg13g2_io/``:

- ``doc/sg13g2_io.celllist``: Available cells tables
